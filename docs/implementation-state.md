...
Current phase: 3
Current slice: 3C (priority overrides + findings) implemented and committed 2026-09-25. Phase 3 slices 3A-3C done; 3D (CLI fixture report + package aggregation) not started. Developer verification of 3A-3C and the Phase 3 gate pending.
...
- Phase 3 Slice 3B (2026-09-25), implemented and tests pass, developer verification pending:
  - `engine/Priority` enum (UNASSESSED, LOW, MODERATE, HIGH, VERY_HIGH; `atLeast` for overrides), `engine/RiskFactorCode` enum, `engine/FactorDefinition` record (code, low, high, weight; `range()`, `clampedExcess(x)`).
  - `engine/StructuralMeasurements` record: the five raw ints; `of(file, graph)` reads maxMethodComplexity, ncloc, `JavaFileAnalysis.maxMethodNcloc()` (new derived method, 0 with no methods, null unparsed), fanOut, fanIn.
  - `engine/RiskFactor` record (raw, thresholds, normalized BigDecimal scale 10, weight, contribution BigDecimal scale 10, explanation) with 0..1 and contribution ≤ weight checks. `engine/AdditionalSignal` (code, count ≥ 1). `engine/RiskAssessment` record (relativePath, policyVersion, graphMode, Integer score, priority, factors, priorityOverrides, additionalSignals, limitations): score/priority/factors all present or all absent; limitations never empty.
  - `engine/StructuralRiskPolicy`: `POLICY_VERSION = "structural-v1"`, `FACTORS` table (50/20/10/10/10 with Blueprint 10.1 thresholds), `normalize`, `factors(measurements)`, `score(measurements)` computed EXACTLY as an integer numerator over lcm(ranges) = 6000 and rounded HALF_UP once, `bandFor(score)`, `assess(file, graph)` → UNASSESSED with a "did not parse (CODE at line N)" limitation for failed files; TODO/FIXME as additional signals; extra limitation when executableCount is 0.
  - `AnalysisResult` gains `assessments` (one per eligible file, same order; `assessment(path)`), invariant: parsed ⇔ scored. `JavaSourceAnalyzer` scores after linking. `AnalyzeMain` prints score/priority columns and top-5 hotspots with factor explanations.
  - `StructuralRiskPolicyTest` 11 tests: exact 59-point PaymentService example (factors 0.7/0.6/0.6/0.4/0.2 → 35/12/6/4/2), clamping at 0/low/high/beyond, all band boundaries (24|25, 49|50, 74|75) from hand-built measurement combinations, half-point exactness (4.5→5, 3.5→4, 6.5→7 where a decimal sum gives 6.4999999999→6), monotonicity over complexity and fanIn, weights sum to 100, real files (Small.java all-below-low → 0 LOW with TODO signal; six importers → fanIn 6 → contribution 0.6666666667 → score 1; Broken.java UNASSESSED; interface-only file explains zero complexity), record shape invariants.

- Phase 3 Slice 3C (2026-09-25), implemented and tests pass, developer verification pending:
  - `StructuralRiskPolicy.overrides(measurements)`: `MIN_HIGH_EXTREME_METHOD_COMPLEXITY` (complexity ≥ 30) and `MIN_HIGH_VERY_LARGE_FILE` (ncloc ≥ 2000); `assess` raises priority via `Priority.atLeast(HIGH)`, score unchanged, codes listed in `priorityOverrides`.
  - `engine/FindingSeverity` (INFO, WARNING, HIGH), `engine/Finding` record (relativePath, ruleCode, severity, nullable begin/end line pair, message ≤ 500 chars, sorted typed evidence map).
  - `engine/FindingRules.evaluate(file, importEvidence)` (`findings-v1`): PARSE_FAILED (WARNING, only finding for failed files; evidence diagnosticCode/count/line); per method EXTREME_METHOD_COMPLEXITY ≥ 30 HIGH else HIGH_METHOD_COMPLEXITY ≥ 20 WARNING, LONG_METHOD ncloc ≥ 80 WARNING (method line range; evidence ownerLabel, signature, value, threshold); VERY_LARGE_FILE ≥ 2000 HIGH else LARGE_FILE ≥ 1000 WARNING; FIXME_MARKER WARNING and TODO_MARKER INFO with counts; UNRESOLVED_IMPORTS INFO with count/examples/truncated. Per file: whole-file findings first by rule, then by line then rule.
  - `AnalysisResult` gains `findings` (flat, path order because files are visited in path order; `findings(path)`). `JavaSourceAnalyzer` evaluates rules after scoring. `AnalyzeMain` prints override codes on hotspots, a per-rule findings tally, and non-INFO findings.
  - `FindingRulesTest` 8 tests with generated fixtures (19/29-branch methods → complexity 20/30 with exact line ranges; 18 branches silent; 80-ncloc method with blank lines interleaved; 1000/2000-ncloc files incl. the override raising a 20-point file to HIGH; markers and unresolved imports as notes with no comment text; failed file; cross-file ordering; record guards). `StructuralRiskPolicyTest` +1 (override cases, `atLeast`, complexity-30 override numerically redundant vs 2000-ncloc override decisive).
...
- 2026-09-25 `./mvnw -B test` (Slice 3B, first run) → BUILD SUCCESS, `Tests run: 100, Failures: 0, Errors: 0` (StructuralRiskPolicyTest 11/11).
- 2026-09-25 Mutation check (score = sum of 10-decimal display contributions, HALF_UP): first attempt SURVIVED — my case 2.5 + 1.3333333333 + 0.6666666667 = 4.5000000000 because the two rounding errors cancelled. Added (11, 300, 0, 7, 7): exact 6.5 → 7, decimal sum 6.4999999999 → 6; mutant then failed `expected: <7> but was: <6>`. Restored → 100/100.
- 2026-09-25 `AnalyzeMain .` hotspots: top score 15 LOW `JavaFileAnalysis.java` (complexity 16 → 15 of 50), then MethodMeasurement 10, LineMetricsCalculator 8 (complexity 13 → 7.5), ParseMain 5 (fanOut 12 → 4.6666666667, longest method 43 → 0.375), ImportEvidence 5. Nothing in this codebase reaches MODERATE.
- 2026-09-25 `./mvnw -B test` (Slice 3C, first run) → BUILD FAILURE, 3 failures, all wrong hand expectations: `mid` endLine expected 25 was 26 (forgot the `int r = 0;` line; `deep` begins 27); 1000-ncloc file expected MODERATE was LOW (20 points is LOW); LONG_METHOD expected but body had 79 ncloc (double-counted the class line). Fixed expectations → `Tests run: 109, Failures: 0, Errors: 0`, BUILD SUCCESS.
- 2026-09-25 Mutation checks (3C): `LONG_METHOD` `>=` → `>` → `expected: <[LONG_METHOD]> but was: <[]>` (the 80-ncloc boundary case); overrides removed from `assess` → `expected: <HIGH> but was: <LOW>` (2000-ncloc file); restored → `Tests run: 109, Failures: 0, Errors: 0`.
- 2026-09-25 `AnalyzeMain .` findings: 64 = FIXME_MARKER 3, PARSE_FAILED 3, TODO_MARKER 5, UNRESOLVED_IMPORTS 53; no complexity/size findings in this codebase. Two FIXME findings are Javadoc sentences mentioning the word (CommentMarkerCounter, RiskAssessment): the word-based counter cannot tell.

## Known failures or limitations
- Slice 3C: finding severities where the contract is silent are my choice (see decisions). Marker findings carry counts only, no line numbers (the counter does not record positions). UNRESOLVED_IMPORTS fires on nearly every real file (JDK imports) at INFO; noisy but per contract. Findings are not capped at 10,000 yet (API: cap + truncation warning) — Phase 8 when run warnings exist. Package aggregation, impact BFS, health profile percentiles, and the CLI fixture report (3D) are not built.
- Slice 3B: with v1 weights the complexity ≥ 30 override never changes a band (30 → 50 points = HIGH already); only the 2000-ncloc override is decisive. Recorded as an observation, not a deviation.
...
- 2026-09-25 Score arithmetic: exact integer numerator over lcm of factor ranges (6000), one HALF_UP rounding via `BigDecimal.divide(..., 0, HALF_UP)`. Displayed normalized/contribution values are 10-decimal HALF_UP approximations (schema `numeric(12,10)` / `numeric(14,10)`, comment 8: "stored decimals are explanatory approximations"). Alternative rejected: summing doubles or rounded decimals (mutation check proved it rounds 6.5 wrongly).
- 2026-09-25 v1 factor raw values, thresholds, and weights are `int` in the engine records (all v1 measurements are integers); Phase 5 maps them to the schema's numeric columns.
- 2026-09-25 Limitations strings are engine-owned constants (import-graph blind spots, history not collected, no bodies → 0 explanation). Additional signals are only emitted when count ≥ 1.
- 2026-09-25 Finding severities (contract lists rules, not severities): EXTREME_METHOD_COMPLEXITY and VERY_LARGE_FILE HIGH; HIGH_METHOD_COMPLEXITY, LONG_METHOD, LARGE_FILE, FIXME_MARKER, PARSE_FAILED WARNING; TODO_MARKER and UNRESOLVED_IMPORTS INFO. PARSE_FAILED is WARNING not HIGH because it is a data-availability problem, not evidence of risky code.
- 2026-09-25 Override codes `MIN_HIGH_EXTREME_METHOD_COMPLEXITY` / `MIN_HIGH_VERY_LARGE_FILE` (API `priorityOverrides: string[]` leaves naming open). Findings rule set versioned separately as `findings-v1` (`FindingRules.RULES_VERSION`); thresholds are constants in that class.
- 2026-09-25 Findings live in a flat `AnalysisResult.findings` list (mirrors the schema's separate `finding` table) rather than inside `RiskAssessment`.
